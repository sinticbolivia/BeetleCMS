<?php
$back_link = $type != 'page' ? $this->Route('index.php?mod=content&type='.$type) : $this->Route('index.php?mod=content');
...
<div class="wrap">
			...
			<div class="row">
				...
				<div class="col-xs-12 col-sm-6 col-md-6 col-lg-6">
					<div class="page-buttons">
						<a href="<?php print $back_link; ?>" class="btn btn-danger"><?php _e('Cancel', 'content'); ?></a>
	...
	<form id="form-content" action="" method="post">
		...
		<div class="row">
			<div id="article" class="col-md-9">
				<div class="row">
					<div class="col-md-8">
						<div class="form-group">
							<label class="has-popover" data-content="<?php print $this->__('CONTENT_TITLE'); ?>">
								<?php print $this->__('Title:', 'content'); ?>
							</label>
							<div class="<?php print $input_class; ?> article-color">
								<input type="text" name="title" value="<?php print $this->request->getString('title', isset($content) ? $content->title : ''); ?>" 
										class="form-control" />
							    ...
							    <a href="<?php print $this->Route('index.php?mod=content&view=article&id='.$content->content_id, 'frontend'); ?>" 
				...
				<div class="form-group">
					<label class="has-popover" data-content="<?php print $this->__('CONTENT_LABEL_UPLOAD_IMG_CHECK', 'content'); ?>">
						<?php print $this->__('Use image intead text button', 'content'); ?>
					...
					<div>
						...
						<div id="button-image" style="<?php print (isset($content) && $content->_button_image) ? '' : 'display:none'; ?>">
							...
							<a href="javascript:;" id="remove-button-image" class="remove" <?php print (isset($content) && $content->_button_image) ? '' : 'style="display:none;"'; ?>>
								<img src="<?php print BASEURL ?>/images/close_window-48x48.png" alt="" title="<?php print $this->__('Eliminar Banner', 'conent')?>" />
							</a>
						</div>
					</div>
				</div>
				...
				<div id="article-banner">
					<label class="has-popover" data-content="<?php print SBText::_('CONTENT_BANNER'); ?>"><?php print $this->__('Banner:', 'content'); ?></label>
					<span class="help-block">
						<?php print $this->__('(Recommended Size: 1920x560 / 1920x300)'); ?>
					</span>
					<div>
						...
						<span id="uploading" style="display:none;">
							<img src="<?php print BASEURL; ?>/js/fineuploader/loading.gif" alt=""  /><?php print $this->__('Uploading image', 'content'); ?>
						</span>
					</div>
					<div id="the-banner">
				...
				<?php endif; ?>
				<div class="form-group">
					<label class="has-popover" data-content="<?php print $this->__('CONTENT_CONTENT'); ?>">
				...
				<?php endif; ?>
				<div class="form-group">
					<textarea id="content_area" name="content" class="form-control"><?php print $this->request->getString('content', isset($content) ? stripslashes($content->content) : ''); ?></textarea>
				</div>
				<?php b_do_action('content_data_'.$type, isset($content) ? $content : null); ?>
				<?php b_do_action('content_data', isset($content) ? $content : null); ?>
				
			</div>
			<div id="sidebar" class="col-md-3">
				<div class="panel panel-default widget">
					<div class="panel-heading">
						<h2 class="panel-title"><?php print $this->__('Options', 'content'); ?></h2>
					</div>
					<div class="panel-body">
						<div class="form-group">
							<label class="has-popover" data-content="<?php print $this->__('CONTENT_STATUS'); ?>">
                                <?php print $this->__('Estado:', 'content'); ?>
                            </label>
							<select name="status" class="form-control">
								<option value="publish" <?php print (isset($content) && $content->status == 'publish') ? 'selected' : ''; ?>>
                                    <?php print $this->__('Published', 'content'); ?></option>
								<option value="draft" <?php print (isset($content) && $content->status == 'draft') ? 'selected' : ''; ?>>
                                    <?php print $this->__('Draft', 'content'); ?></option>
							</select>
						</div>
						<div class="form-group">
						...
						<div class="form-group">
							<label class="has-popover" data-content="<?php print $this->__('CONTENT_PUBLSIH_DATE'); ?>">
								<?php print $this->__('Publish Date:', 'content'); ?>
							</label>
						...
						<div class="form-group">
							<label class="has-popover" data-content="<?php print $this->__('CONTENT_EXPIRES_DATE'); ?>">
								<?php print $this->__('Expiration Date:', 'content'); ?>
							</label>
						...
						<div class="row">
							<div class="col-md-12 form-group">
								<label class="has-popover" data-content="<?php print $this->__('CONTENT_PUBLISH_C_DATE'); ?>">
                                    <?php print $this->__('Calculated Publish Date:', 'content'); ?></label><br/>
						...
						<div class="row">
							<div class="col-md-12 form-group">
								<label class="has-popover" data-content="<?php print $this->__('CONTENT_EXPIRES_C_DATE'); ?>">
                                    <?php print $this->__('Calculated Expiration Date:', 'content'); ?></label><br/>
								<div class="row col-md-5">
						...
						<div class="form-group">
							<label class="has-popover" data-content="<?php print $this->__('CONTENT_USE_CDATES'); ?>">
								<input type="checkbox" name="use_calculated" value="1" <?php print (isset($content) && $content->_use_calculated) ? 'checked' : '';?> /> 
								<?php print $this->__('Use calculated dates', 'content')?>
							</label>
						</div>
						<?php endif; ?>
						<?php if( $type == 'page' ): ?>
						...
						<?php endif; ?>
						<?php b_do_action('content_options', isset($content) ? $content : null); ?>
						<p class="text-center">
							<a class="btn btn-danger has-popover" href="<?php print $back_link; ?>"
								data-content="<?php print $this->__('CONTENT_CANCEL'); ?>">
								<?php print $this->__('Cancel', 'content'); ?></a>
							<button type="submit" class="btn btn-success has-popover" data-content="<?php print $this->__('CONTENT_SAVE'); ?>">
								<?php print $this->__('Save', 'content'); ?>
                            </button>
						</p>
					</div>
				</div>
				<?php b_do_action('after_article_options', isset($content) ? $content : null); ?>
				<?php if( isset($content) && $content->type == 'page' || (!isset($content) && $type == 'page') ): ?>
				<div class="panel panel-default">
					<div class="panel-heading">
						<h2 class="panel-title has-popover" data-content="<?php print $this->__('CONTENT_WH_SECTIONS'); ?>">
							<?php print $this->__('Template Options', 'content'); ?>
						</h2>
					</div>
					<div class="panel-body">
				...
				<div class="panel panel-default">
					<div class="panel-heading">
						<h2 class="panel-title has-popover" data-content="<?php print $this->__('CONTENT_WH_CATEGORIES'); ?>">
				...
				<div class="panel panel-default">
					<div class="panel-heading">
						<h2 class="panel-title has-popover" data-content="<?php print $this->__('CONTENT_WH_FEATURED_IMAGE'); ?>">
				...
				<?php endif; ?>
				<?php b_do_action('article_sidebar', isset($content) ? $content : null); ?>
				<?php b_do_action('content_sidebar', isset($content) ? $content : null); ?>
				<?php b_do_action('content_'.$type.'_sidebar', isset($content) ? $content : null); ?>
			</div>
		</div>
	</form>
	...
	jQuery(function()
	{
		...
		var button_uploader = new qq.FineUploaderBasic({
			...
			request: {
				endpoint: '<?php print $this->Route('index.php?mod=content&task=upload_button_image' . (isset($content) ? '&id='.$content->content_id : '')); ?>'
			},
			validation: {
				allowedExtensions: ['jpeg', 'jpg', 'gif', 'png']
			},
		...
		jQuery('input[name=qqfile]').attr('title', '<?php print $this->__('Sube una imagen de tu equipo')?>');
	...
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			...
			<div class="modal-body">
				<iframe id="iframe-upload-media" src="" data-src="<?php print $this->Route('index.php?mod=storage&view=uploader&tpl_file=module'); ?>" 
