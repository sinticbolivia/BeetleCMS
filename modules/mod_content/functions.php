...
function lt_content_rewrite($raw_url, $components)
{
	...
	elseif( $components['view'] == 'category' )
	{
		
		$slug = !isset($components['slug']) && isset($components['id']) ? 
					$dbh->GetVar("SELECT slug FROM categories WHERE category_id = {$components['id']} LIMIT 1") : 
					$components['slug'];
		if( empty( $slug ) )
			return $raw_url;
		//var_dump(BASEURL . '/' . $slug);
		return BASEURL . '/' . __('category', 'content') . '/' . $slug;
		//', 'content') . '/' . $slug;
	}
	else
	{
...
{
	return b_get_frontpage_items($limit);
}
/**
 * Get frontpage pages
 * 
 * @param number $limit
 * @return Array<LT_Article> The pages
 */
function b_get_frontpage_items($limit = 6)
{
...
/**
 * 
 * Register content types for Beetle CMS
 * 
 */
