...
class LT_AdminControllerContentSection extends SB_Controller
{
	public function task_default()
	{
		...
		$fo			= $this->request->getString('fo', 'page');
		...
		*/					
		...
		if( $fo != 'page' )
		{
			...
			$link_btn_new	= $this->Route('index.php?mod=content&view=section.new&fo='.$fo);
		}
	...
	public function task_edit()
	{
		...
		$section->SetDbData($this->dbh->FetchRow());
	...
	public function task_save()
	{
		...
		$publish_date           = $this->request->getString('publish_date');
		$end_date               = $this->request->getString('end_date');
		$calculated_date        = $this->request->getInt('calculated_date');
		$calculated_end_date    = $this->request->getInt('calculated_end_date');
		...
		*/
		...
		if( !$section_id )
		{
			...
			$section_id = $this->dbh->Insert('section', $data);
			//##add section meta
			...
			sb_add_section_meta($section_id, '_use_calculated_dates', $this->request->getInt('use_calculated_dates'));
			$msg    = SB_Text::_('La secci&oacute;n fue creada correctamente.', 'content');
			$link   = $this->Route('index.php?mod=content&view=section.default&fo='.$fo);
		}
		else 
		{
			...
			$this->dbh->Update('section', $data, array('section_id' => $section_id));
			//##add section meta
			...
			sb_update_section_meta($section_id, '_use_calculated_dates', $this->request->getInt('use_calculated_dates'));
			
			$msg = $this->__('La secci&oacute;n fue actualizada correctamente.', 'content');
			$link = $this->Route('index.php?mod=content&view=section.edit&id='.$section_id);
		}
		sb_update_section_meta($section_id, '_btn_bg_color', $this->request->getString('btn_bg_color', '#000'));
		sb_update_section_meta($section_id, '_btn_fg_color', $this->request->getString('btn_fg_color', '#000'));
		sb_update_section_meta($section_id, '_use_button_instead', $this->request->getInt('use_button_instead'));
	...
	public function task_delete()
	{
		...
		$section_id = $this->request->getInt('id');
		if( !$section_id )
		{
