...
class LT_ControllerForms extends SB_Controller
{
	...
	public function task_send()
	{
		...
		if( !$id )
		{
			SB_MessagesStack::AddMessage(__('The form identifier is invalid', 'forms'), 'error');
		...
		*/
		...
		if( !$form->FillFields() )
		{
			SB_MessagesStack::AddMessage(__('There is errors on your form submit, please double check.', 'forms'), 'error');
			SB_Session::setVar('form_data', $_POST);
			sb_redirect($_SERVER['HTTP_REFERER']);
			return false;
		}
