...
class AdminController extends SB_Controller
{
	...
	public function task_new_user()
	{
		...
		$this->document->SetTitle(__('Create New User', 'users'));
	...
	public function task_edit_user()
	{
		...
		$this->document->SetTitle(__('Edit User', 'users'));
		sb_add_script(BASEURL . '/js/fineuploader/all.fine-uploader.min.js', 'fineuploader');
		...
		sb_set_view_var('title', SB_Text::_('Edit User', 'users'));
	...
	public function task_save_user()
	{
		...
		*/
		...
		{
			SB_MessagesStack::AddMessage(_('You cant use this for role for the user.', 'users'), 'error');
			if( $user_id )
			{
				$this->task_edit_user();
			}
		...
		if( !$current_user->IsRoot() && !$role->role_id )
		{
			...
			SB_MessagesStack::AddMessage(__('The user role does not exists', 'users'), 'error');
			return false;
		}
		if( !$current_user->can('see_all_users') && (int)$current_user->user_id !== (int)$user_id && !$current_user->can('create_role_'.$role->role_key) )
		{
		...
		else
		{
			...
			if( !$user->user_id )
			{
				SB_MessagesStack::AddMessage(__('The user identifier does not exists.', 'users'), 'error');
				return false;
			}
			if( !$current_user->IsRoot() && !$current_user->can('see_all_users') )
			{
				if( $current_user->user_id != $user_id && sb_get_current_user()->role_id !== 0 && sb_get_current_user()->user_id != $user->_owner_id )
				{
					SB_MessagesStack::AddMessage(__('No puede editar el usuario solicitado.', 'users'), 'error');
					sb_redirect(SB_Route::_('index.php?mod=users'));
				}
			}
			...
			SB_Module::do_action('save_user', $user_id);
			//##update user
			SB_MessagesStack::AddMessage(__('The user has been updated', 'users'), 'success');
