...
class SB_User extends SB_ORMObject
{
	...
	public function GetDbPermissions()
	{
		...
		$res = $this->dbh->Query($query);
		$permissions = $this->dbh->FetchResults();
	...
	*/
	...
	public function GetAvatar()
	{
		...
		{
			$image_url = $this->GetDirectoryUrl() . '/' . $image;
		}
    ...
    public function GetDirectory()
    {
        if( !$this->user_id || !$this->username )
            return null;
        return UPLOADS_DIR . SB_DS . sb_build_slug($this->username);
    }
    /**
    ...
    public function GetDirectoryUrl()
    {
        if( !$this->user_id || !$this->username )
            return null;
        return UPLOADS_URL . '/' . sb_build_slug($this->username);
    }
}
